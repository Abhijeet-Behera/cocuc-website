...
with open('events.php', 'r') as f:
    content = f.read()

new_post_handler = """if ($method === 'POST') {
    try {
...
# Using regex to replace the POST block
pattern = re.compile(r"if \(\$method === 'POST'\) \{.*?\n\}\n", re.DOTALL)
new_content = pattern.sub(new_post_handler + "\n\n", content)

with open('events.php', 'w') as f:
    f.write(new_content)
